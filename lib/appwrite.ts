...
export const updateLikedPost = async (videoId: string, userId: string) => {
    //console.log("videoID",videoId);
    //console.log("UserId",userId);

    try {
        const currentVideo = await databases.listDocuments(
            appwriteConfig.databaseId,
            appwriteConfig.videoCollectionId,
            [Query.equal("$id",videoId)]
        )
        const currentLike = currentVideo.documents[0].likes || []
        
        
        if (!currentLike.includes(userId)) {
            const updatedLikes = currentLike.concat(userId)           
            //console.log("updatedLikes",updatedLikes);
            
            const bookmarkedPost = await databases.updateDocument(
                appwriteConfig.databaseId,
                appwriteConfig.videoCollectionId,
                videoId,
                {
            ...
            //console.log("updatedPost",bookmarkedPost);
            return {
                success: true,
...
export const getUserSavedPosts = async (id: string) => {
    try {
        ...
        let bookmarkedVideos:any = []
        const savedVideos = videos.documents.map((video: any)=>{
            if(video.likes.includes(id)) return bookmarkedVideos.push(video)
        })
        //console.log("savedVideo",bookmarkedVideos);
...
export const getSearchUserSavedPosts = async()=>{
